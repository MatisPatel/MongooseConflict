...
function makeRsys(R, F, M, P, C, d, world)
    Rp = makeFArray(world)
...
function genSolR(rFun, world)
    ...
    , autodiff = :forward, method=:anderson)
end
...
function runSim(world)
    ...
    rSys = Symbolics.scalarize(makeRsys(R, F, M, P, C, d, world));
...
function re_evaluate_world(world, ratio)

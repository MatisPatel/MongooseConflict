...
```{r, include=FALSE}
dat <- read_csv('../results/02082021_35_53.csv') %>% 
...
ggplot(plotDat %>% filter(force==0.03)) +
  facet_wrap(~Trait, scales="free") +
  geom_path(aes(log2(EncounterRate), 
                Investment, 
                linetype=Harshness), size=1) +
  my_theme
ggsave("../graphs/traits_with_encounter.pdf")
```
...
```
```{r, message=FALSE}
ggplot(plotDat%>%filter(Trait=="Conflict")) + 
  facet_grid(~Trait, scales="free") +
  geom_line(aes(Quality, Investment, 
                linetype=as.factor(Harshness)),
            size=1) +
  my_theme
ggsave("../graphs/conflict_with_quality.pdf")
```
...
```
```{r}
ggplot(plotDat%>%filter(Quality==2)) + 
  facet_wrap(~Trait, scales="free_y") +
  geom_line(aes(Size, Investment, 
                linetype=as.factor(Harshness)),
            size=1) +
  my_theme
ggsave("../graphs/traits_with_size.pdf")
ggplot(plotDat%>%filter(Quality==2)) + 
  facet_wrap(~Trait, scales="free_y") +
  geom_line(aes(Size, Investment*Size, 
            ...
            size=1) +
  my_theme
ggsave("../graphs/grouptraits_with_size.pdf")
```
...
ggplot(plotDat %>% 
  ...
  my_theme
ggsave("../graphs/traits_with_harshness.pdf")
...
ggplot(plotDat) +
  ...
  my_theme
ggsave("../graphs/value_harshness.pdf")
```
...
```{r}
ggplot(td) + 
  geom_line(aes(pHarsh, diffX, colour=as.factor(Harshness)), size =1, alpha=0.7) +
  geom_hline(yintercept=0, alpha =0.5) +
  geom_point(aes(Harshness, rep(0,length(Harshness)), colour=as.factor(Harshness)), shape=4, size=4) +
  my_theme
ggsave("../graphs/perturb_cooperation.pdf")

```

```{r}
ggplot(td) + 
  geom_line(aes(pHarsh, diffY, colour=as.factor(Harshness)), size =1, alpha=0.7) +
  geom_hline(yintercept=0, alpha=0.5) +
  geom_point(aes(Harshness, rep(0,length(Harshness)), colour=as.factor(Harshness)), shape=4, size=4) +
  my_theme
ggsave("../graphs/perturb_conflict.pdf")
```
